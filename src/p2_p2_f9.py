...
for key, list in first_processed_data.items():
    first_dict = Counter(list)
    second_dict = Counter(second_processed_data[key])
    if len(first_dict) == len(second_dict):
        for action in first_dict:
            if first_dict[action] != second_dict[action]:
                print "mismathced", key, action
                print "first data:", first_dict[action]
                print "second data:", second_dict[action]
        for action in second_dict:
            if second_dict[action] != first_dict[action]:
                print "mismathced", key, action
                print "first data:", first_dict[action]
                print "second data:", second_dict[action]
    ...
    if len(first_dict) > len(second_dict):
        for action in first_dict:
            try:
            	...
            	print "This element can not be found at second data", action

    if len(second_dict) > len(first_dict):
        for action in second_dict:
            try:
                if second_dict[action] != first_dict[action]:
                    print "mismathced", key, action
            	...
            	print "This element can not be found at first data", action
