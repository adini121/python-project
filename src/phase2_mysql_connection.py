...
class Phase2MysqlPython(object):
    ...
    def __new__(cls, *args, **kwargs):
        if not cls.__instance or not cls.__database:
             cls.__instance = super(Phase2MysqlPython, cls).__new__(cls,*args,**kwargs)
    ...
    def select_phase2_major_version_database(self, phase2_major_version_database, table):
        """  """

        """ Fireplace """
        if phase2_major_version_database == 'fireplace_mv1': # Select All rows
            query = "SELECT session_id FROM %s" % table

        if phase2_major_version_database == 'fireplace_mv2':
            query = "SELECT session_id FROM %s" % table

        if phase2_major_version_database == 'fireplace_mv3':
